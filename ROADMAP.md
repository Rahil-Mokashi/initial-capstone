...
7. ~~Login screen redesign pass (declutter, bigger text, responsive width, simpler error wording, password auto-hide, hover/press/shake animation "feel alive" pass)~~ — **done 2026-09-25**, from the user's own hands-on review. See PROJECT_CONTEXT.md's "Client-perspective review, item 7" for full detail. Left open, not started: a prioritized menu of further login-page ideas (Caps Lock warning, on-screen device/station name, a theme toggle on the login screen itself, remember-username, attempts-remaining warning, first-login password-change nudge, offline indicator, and three bigger decisions - PIN quick shift-change login, Windows Hello, a regional-language toggle) was handed to the user; none of it is committed work.
