...
- [x] Implement daily/shift/attendant/HR/inventory/management reports enumerated in problemstatement.md #25-32 (done 2026-08-18: **Daily Summary**, **Attendant & Nozzle**, **Fuel Movement**, **Cash Book**, **Attendance/HR** - five cross-module reports on the existing shared TableReport/export/print infrastructure. Management trend reporting remains covered by the existing Business Insights view rather than duplicated.) Original note (daily reports across every module, attendant-wise/nozzle-wise/fuel-wise shift breakdowns, HR reports, tank/fuel-movement inventory reports, cash book/ledger financial reports, trend/exception management reports) - a much larger set than what's built above, deliberately left for a dedicated later pass rather than attempted in the same session as five other phases
- [x] Filter by employee/nozzle (done 2026-08-18): a generic `choice_filters` mechanism on `TableReportWindow`, with the option lists gated per-dimension by `ReportService.get_report_filter_options` so a role that may open a report but not see the staff roster gets no employee drop-down at all
...
## Phase 18: Backup & Recovery (Complete except recovery-doc and optional encryption)
...
- [x] ~~Configure backup location (local disk, external USB, network folder)~~ — **done** (`copy_backup_offsite` in `app/services/backup_service.py`, "Copy to USB/network..." on the Backups screen, `offsite_backup_dir` on the Settings screen to remember the chosen folder, a staleness warning if the last off-device copy is old or missing). Every automatic/scheduled backup still lands in the fixed `<db_dir>/backups` folder first - the configurable part is where a *copy* goes for off-device safekeeping, which is what actually protects against a dead drive, theft or fire
...
- [x] Create installer package (`installer/petrol_pump_erp.iss`, Inno Setup — Start Menu entry, optional desktop shortcut, registered uninstaller; see `installer/README.md`)
- [x] Implement configuration system (`app/core/config.py`'s `Settings` — `session_timeout_hours` and `auto_backup_interval_hours` are now real, wired, user-editable settings, not decorative fields or hardcoded constants; editable via `%LOCALAPPDATA%\PetrolPumpERP\config.env` in a packaged build without a rebuild)
...
- [x] Create default directories (`app/core/paths.py` — backups/reports/logs created eagerly on every startup via `ensure_app_directories()`, not just lazily on first use; report/receipt/statement exports now default their save-as location into `reports/`)
- [x] Package user documentation (`docs/user-guide.md`)
- [x] Package administrator documentation (`docs/administrator-guide.md`)
- [x] Package recovery documentation (`docs/recovery-guide.md`)
- [x] Create release process (`.github/workflows/release.yml` — a pushed `v*.*.*` tag builds the exe and installer and publishes both as a GitHub Release)

Phase 20 is now complete end to end (2026-08-16).
...
## Notifications & Alerts (problemstatement.md #43 — Complete)
All thirteen specified alert types are implemented in `app/services/notification_service.py` + `app/ui/notification_window.py`, surfaced through a self-colouring count in the dashboard top bar.
- [x] Low fuel (per tank, CRITICAL below half the low-stock flag threshold since the pump is then close to stopping sales, not merely due a reorder)
- [x] Fuel variance (latest reconciliation per tank only — re-raising a superseded one would mean a variance could never be cleared by doing the correct thing)
- [x] Cash shortage and cash excess, tracked as **separate** categories: an excess is still a discrepancy (usually an unrecorded sale) but it is not a loss, so it does not carry a shortage's weight
- [x] Payment mismatch (UPI/card variance), failed reconciliation (awaiting approval)
- [x] Attendance issue — reports a **missing** record rather than a bad one: an employee marked absent has been accounted for, while one with no row at all is the gap a supervisor can still close today
- [x] Unauthorized action — required closing a real gap first: `require_permission` raised and kept no record. Rolled up with a count and escalating severity, because one denial is a mis-click and a burst is a pattern
- [x] Pending approval (expenses, shift reconciliations) — gated on the *approve* permission, not the view one
- [x] Outstanding credit (overdue accounts), supplier payment due (past due date, unpaid balance)
- [x] Backup failure — derived from the absence of a recent backup rather than a recorded failure, so it catches every cause including ones nobody added logging to
- [x] Database error — integrity-check failures and audit hash-chain breaks, each now recorded under its own event type rather than inferred from a description string
- [x] Deliberately **no `notifications` table and no "mark as read"** — see PROJECT_CONTEXT.md for the full reasoning; in short, an alert derived from live data cannot go stale, and dismissing one that is still true is a way to hide a real problem
- [x] 42 tests (`tests/test_notification_service.py`, `tests/test_notification_ui.py`)
...
7. ~~Login screen redesign pass (declutter, bigger text, responsive width, simpler error wording, password auto-hide, hover/press/shake animation "feel alive" pass)~~ — **done 2026-09-25**, from the user's own hands-on review. See PROJECT_CONTEXT.md's "Client-perspective review, item 7" for full detail.
8. ~~Full login-page feature list (Caps Lock warning, lockout countdown/attempts-remaining, remembered usernames, device/offline/theme info on login, PIN-based quick sign-in, self-service password reset via admin code, screen-lock quick unlock, Windows Hello, login-screen language toggle)~~ — **done 2026-09-25**. See PROJECT_CONTEXT.md's "Client-perspective review, item 8" for full detail, including why SSO/passkeys/CAPTCHA were deliberately left out (they assume online infrastructure this offline app doesn't have).
