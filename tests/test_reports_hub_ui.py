...
def test_reports_hub_shows_every_report_for_admin(qapp, report_service, auth_service, admin_id):
    from app.ui.report_window import ReportsHubWindow

    window = ReportsHubWindow(report_service, auth_service, admin_id)
    # 7 reports: fuel summary, sales, payment summary, expense summary, credit x2, reconciliation
    assert window.centralWidget() is not None


def test_reports_hub_hides_reports_attendant_cannot_view(qapp, report_service, auth_service, attendant_id):
    from app.ui.report_window import ReportsHubWindow

    window = ReportsHubWindow(report_service, auth_service, attendant_id)
